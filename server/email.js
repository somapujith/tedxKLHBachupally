...
// Marketing nudge for anyone who registered interest but never completed
// payment (payment_status pending/submitted/rejected) — the Early Bird rate
// is a moving target set at runtime (server/settings.js), so this always
// takes `price` as an argument rather than assuming ₹449.
export function earlyBirdHtml({ fullName, price }) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8" />
<meta name="viewport" content="width=device-width,initial-scale=1" />
<title>Early Bird pricing ends soon — TEDxKLH Bachupally</title>
</head>
<body style="margin:0;padding:0;background:#f4f4f4;">
  <div style="display:none;max-height:0;overflow:hidden;opacity:0;">Your TEDxKLH Bachupally seat is still open — Early Bird pricing won't last.</div>
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f4f4f4;">
    <tr>
      <td align="center" style="padding:32px 16px;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:560px;background:#ffffff;border-radius:14px;overflow:hidden;">
          ...
          <tr>
            <td style="padding:36px 32px 6px;">
              <div style="display:inline-block;padding:5px 12px;border-radius:999px;background:#fdf2f1;font:700 11px/1.2 Arial,Helvetica,sans-serif;letter-spacing:.08em;text-transform:uppercase;color:#b3261e;">Early Bird closes soon</div>
              <h1 style="margin:16px 0 0;font:700 26px/1.3 Arial,Helvetica,sans-serif;color:#111111;">Your seat at ${event.edition} is still open</h1>
              <p style="margin:14px 0 0;font:400 15px/1.6 Arial,Helvetica,sans-serif;color:#4a4a4a;">Hi ${fullName}, you started registering for TEDxKLH Bachupally but haven't completed your pass yet. Early Bird pricing is going up soon — lock in your seat at today's rate before it does.</p>
            </td>
          </tr>
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#fdf2f1;border-left:3px solid #e62b1e;border-radius:0 8px 8px 0;">
                <tr>
                  <td style="padding:16px 18px;">
                    <span style="font:600 11px/1.2 Arial,Helvetica,sans-serif;letter-spacing:.09em;text-transform:uppercase;color:#8a8a8a;">Early Bird rate, today only</span>
                    <div style="margin-top:4px;font:700 28px/1.2 Arial,Helvetica,sans-serif;color:#b3261e;">&#8377;${price}</div>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td align="center" style="padding:30px 32px 8px;">
              <a href="${SITE_URL}/register" style="display:inline-block;padding:15px 36px;border-radius:8px;background:#e62b1e;font:700 14px/1.2 Arial,Helvetica,sans-serif;letter-spacing:.03em;color:#ffffff;text-decoration:none;">Complete your registration &rsaquo;</a>
            </td>
          </tr>
          <tr>
            <td align="center" style="padding:0 32px 34px;">
              <p style="margin:0;font:400 12px/1.5 Arial,Helvetica,sans-serif;color:#9a9a9a;">Takes under two minutes. Already paid? Ignore this — you're all set.</p>
            </td>
          </tr>
...
export function earlyBirdText({ fullName, price }) {
  return [
    'TEDxKLH BACHUPALLY',
    '',
    'EARLY BIRD CLOSES SOON',
    '',
    `Hi ${fullName}, you started registering for TEDxKLH Bachupally but haven't`,
    "completed your pass yet. Early Bird pricing is going up soon — lock in",
    "today's rate before it does.",
    '',
    `Early Bird rate, today only: Rs ${price}`,
    '',
    `Date:  ${event.date}`,
    `Time:  ${event.time} (${event.timeNote})`,
    `Venue: ${event.venue}, ${event.streetAddress}, ${event.locality}, ${event.region} ${event.postalCode}`,
    '',
    `Complete your registration: ${SITE_URL}/register`,
    '',
    "Already paid? Ignore this — you're all set.",
    '',
    `${contact.email} · ${contact.instagramHandle}`,
  ].join('\n')
}

// Never throws — same contract as sendBookingEmail/sendTicketEmail. Caller
// decides what "failed" means for a batch send (log and move on, typically).
export async function sendEarlyBirdEmail({ to, fullName, price }) {
  const apiKey = process.env.RESEND_API_KEY
  if (!apiKey) {
    console.warn('RESEND_API_KEY not set — skipping early bird email for', to)
    return { ok: false, skipped: true }
  }
  ...
  try {
    const { data, error } = await getClient(apiKey).emails.send({
      from: fromAddress(),
      to: [to],
      subject: 'Early Bird pricing ends soon — TEDxKLH Bachupally',
      html: earlyBirdHtml({ fullName, price }),
      text: earlyBirdText({ fullName, price }),
      attachments: [
        { filename: 'tedx-logo.png', content: TEDX_LOGO_PNG_BASE64, contentType: 'image/png', contentId: LOGO_CID },
      ],
    })
    ...
    if (error) {
      console.error('Early bird email rejected by Resend:', error.name, error.message)
      return { ok: false, error: 'Email send failed.', detail: `${error.name}: ${error.message}` }
    }
    if (!data?.id) {
      console.error('Early bird email returned no message id.')
      return { ok: false, error: 'Email send failed.', detail: 'Provider returned no message id.' }
    }
    return { ok: true, id: data.id }
  } catch (err) {
    console.error('Early bird email send failed:', err?.message || err)
    return { ok: false, error: 'Email send failed.', detail: String(err?.message || err) }
  }
}
